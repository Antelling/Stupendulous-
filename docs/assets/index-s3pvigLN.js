var ht=Object.defineProperty;var ct=(h,t,e)=>t in h?ht(h,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):h[t]=e;var o=(h,t,e)=>ct(h,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function e(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=e(s);fetch(s.href,a)}})();const tt=["angle1","velocity1","angle2","velocity2"],et=["angle1","velocity1","stretch1","stretchRate1","angle2","velocity2","stretch2","stretchRate2"],V=["angle1","velocity1","stretch1","stretchRate1","angle2","velocity2","stretch2","stretchRate2"],dt={angle1:Math.PI,velocity1:5,angle2:Math.PI,velocity2:5,stretch1:.5,stretchRate1:5,stretch2:.5,stretchRate2:5};function Z(h){const t=new Array(V.length).fill(0);return t[V.indexOf(h)]=1,t}function it(h){const t=h.phaseSpace.initialValues;return V.map(e=>t[e])}function Y(h,t=0,e=0){const i=h.phaseSpace;if(i.mode==="tiling"){const l=i.tiling,u=l.cols,c=l.rows,f=t,d=l.toroidal?(t+1)%u:Math.min(t+1,u),m=e,p=l.toroidal?(e+1)%c:Math.min(e+1,c),g=(y,T)=>l.controlNet[T][y],b=g(f,m),_=g(d,m),A=g(f,p),x=g(d,p);return[b.slice(),_.slice(),A.slice(),x.slice()]}const s=it(h),a=Z(i.x.dimension),r=Z(i.y.dimension),n=(l,u,c)=>l.map((f,d)=>f+u*a[d]+c*r[d]);return[n(s,i.x.min,i.y.min),n(s,i.x.max,i.y.min),n(s,i.x.min,i.y.max),n(s,i.x.max,i.y.max)]}function J(h,t,e){const[i,s,a,r]=h,n=new Array(i.length);for(let l=0;l<i.length;l++)n[l]=(1-t)*(1-e)*i[l]+t*(1-e)*s[l]+(1-t)*e*a[l]+t*e*r[l];return n}function z(h){return[h[0],h[1],h[4],h[5]]}function H(h){return[h[0],h[1],h[2],h[3]]}function U(h){return[h[4],h[5],h[6],h[7]]}function st(h,t,e,i){const a=(h==="rigid"?tt:et).map(n=>V.indexOf(n)),r=[];for(let n=0;n<e;n++){const l=[];for(let u=0;u<t;u++){const c=i.slice();for(const f of a){const d=V[f];c[f]=i[f]+(Math.random()*2-1)*dt[d]}l.push(c)}r.push(l)}return{cols:t,rows:e,toroidal:t>=2&&e>=2,controlNet:r}}function ft(h){return`${h.cols}×${h.rows}${h.toroidal?" torus":""}`}const mt={system:"rigid",vizMode:"distance",resolution:512,chunkSize:512,phaseSpace:{x:{dimension:"angle1",min:-Math.PI,max:Math.PI},y:{dimension:"angle2",min:-Math.PI,max:Math.PI},initialValues:{angle1:0,velocity1:0,angle2:0,velocity2:0,stretch1:0,stretchRate1:0,stretch2:0,stretchRate2:0},mode:"manual",tiling:st("rigid",2,2,[0,0,0,0,0,0,0,0])},dt:.002,iterationsPerFrame:10,maxIter:5e3,perturb:1e-5,perturbDistribution:"uniform",trials:1,m1:1,m2:1,L1:1,L2:1,k1:50,k2:50,colormap:6,toneMapping:0,seed:Math.random()*1e3},pt={rigid:"Rigid",elastic12:"Elastic Double Pendulum",nonlinear:"Nonlinear Elastic Pendulum"},gt={distance:"Bob2 Distance",divergence:"Divergence Time",divergenceDistance:"Divergence Distance"};class bt{constructor(t){o(this,"buffer");this.gl=t;const e=t.createBuffer();if(!e)throw new Error("Failed to create buffer");this.buffer=e;const i=new Float32Array([-1,-1,1,-1,-1,1,1,1]);t.bindBuffer(t.ARRAY_BUFFER,this.buffer),t.bufferData(t.ARRAY_BUFFER,i,t.STATIC_DRAW)}bind(){this.gl.bindBuffer(this.gl.ARRAY_BUFFER,this.buffer)}draw(){this.gl.drawArrays(this.gl.TRIANGLE_STRIP,0,4)}}class at{constructor(t){this.gl=t}createFloatTexture(t){const e=this.gl.createTexture();if(!e)throw new Error("Failed to create texture");return this.gl.bindTexture(this.gl.TEXTURE_2D,e),this.gl.texImage2D(this.gl.TEXTURE_2D,0,this.gl.RGBA32F,t,t,0,this.gl.RGBA,this.gl.FLOAT,null),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_MIN_FILTER,this.gl.NEAREST),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_MAG_FILTER,this.gl.NEAREST),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_WRAP_S,this.gl.CLAMP_TO_EDGE),this.gl.texParameteri(this.gl.TEXTURE_2D,this.gl.TEXTURE_WRAP_T,this.gl.CLAMP_TO_EDGE),e}createTexturePair(t){return[this.createFloatTexture(t),this.createFloatTexture(t)]}bindTexture(t,e){this.gl.activeTexture(this.gl.TEXTURE0+t),this.gl.bindTexture(this.gl.TEXTURE_2D,e)}}class rt{constructor(t){this.gl=t}getLocation(t,e){return this.gl.getUniformLocation(t,e)}setIfFound(t,e,i){const s=this.getLocation(t,e);s!==null&&i(s)}set1f(t,e,i){this.setIfFound(t,e,s=>this.gl.uniform1f(s,i))}set1i(t,e,i){this.setIfFound(t,e,s=>this.gl.uniform1i(s,i))}set2f(t,e,i,s){this.setIfFound(t,e,a=>this.gl.uniform2f(a,i,s))}set4f(t,e,i,s,a,r){this.setIfFound(t,e,n=>this.gl.uniform4f(n,i,s,a,r))}set1b(t,e,i){this.setIfFound(t,e,s=>this.gl.uniform1i(s,i?1:0))}}class nt{constructor(t){this.gl=t}create(){const t=this.gl.createFramebuffer();if(!t)throw new Error("Failed to create framebuffer");return t}attachColor(t,e){this.gl.framebufferTexture2D(this.gl.FRAMEBUFFER,t,this.gl.TEXTURE_2D,e,0)}checkStatus(){const t=this.gl.checkFramebufferStatus(this.gl.FRAMEBUFFER);if(t!==this.gl.FRAMEBUFFER_COMPLETE)throw new Error(`Framebuffer incomplete: ${t}`)}bind(t){this.gl.bindFramebuffer(this.gl.FRAMEBUFFER,t)}}class N{constructor(t){this.gl=t}compile(t,e){const i=this.gl.createShader(t);if(!i)throw new Error("Failed to create shader");if(this.gl.shaderSource(i,e.trim()),this.gl.compileShader(i),!this.gl.getShaderParameter(i,this.gl.COMPILE_STATUS)){const s=this.gl.getShaderInfoLog(i);throw this.gl.deleteShader(i),new Error(`Shader compile error: ${s}`)}return i}linkProgram(t,e,i){const s=this.compile(this.gl.VERTEX_SHADER,t),a=this.compile(this.gl.FRAGMENT_SHADER,e),r=this.gl.createProgram();if(!r)throw new Error("Failed to create program");if(this.gl.attachShader(r,s),this.gl.attachShader(r,a),this.gl.linkProgram(r),this.gl.deleteShader(s),this.gl.deleteShader(a),!this.gl.getProgramParameter(r,this.gl.LINK_STATUS)){const n=this.gl.getProgramInfoLog(r);throw this.gl.deleteProgram(r),new Error(`Program link error (${i}): ${n}`)}return{program:r,name:i}}}const k=`// Rigid double pendulum ODE right-hand side
...
`,B=`// Compute bob2 position given angles and arm lengths
...
`}static rigidDistanceInit(){return`${v}
uniform vec4 u_c00;
uniform vec4 u_c10;
uniform vec4 u_c01;
uniform vec4 u_c11;
...
}`}static rigidDivergenceInit(){return`${v}
uniform vec4 u_c00;
uniform vec4 u_c10;
uniform vec4 u_c01;
uniform vec4 u_c11;
...
}`}static elasticDistanceInit(){return`${v}
uniform vec4 u_cA00;
uniform vec4 u_cA10;
uniform vec4 u_cA01;
uniform vec4 u_cA11;
...
}`}static nonlinearDistanceInit(){return this.elasticDistanceInit()}static nonlinearDivergenceInit(){return`${v}
uniform vec4 u_cA00;
uniform vec4 u_cA10;
uniform vec4 u_cA01;
uniform vec4 u_cA11;
...
void main() {
    ...
    divergenceData = vec4(0.0, 0.0, 0.0, 1.0);
}`}static elasticDivergenceInit(){return`${v}
uniform vec4 u_cA00;
uniform vec4 u_cA10;
uniform vec4 u_cA01;
uniform vec4 u_cA11;
...
}`}static elasticPhysics(){return`${v}
...
}`}static nonlinearPhysics(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform float u_dt;
uniform float u_k1;
...
void main() {
    ...
    outStateA = sa + (dt / 6.0) * (da1 + 2.0*da2 + 2.0*da3 + da4);
    outStateB = sb + (dt / 6.0) * (db1 + 2.0*db2 + 2.0*db3 + db4);
}`}static rigidAccumulate(){return`${v}
uniform sampler2D u_stateTexture;
...
}`}static elasticAccumulate(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform sampler2D u_distanceTexture;
uniform bool u_reset;
...
void main() {
    ...
    float newDist = (u_reset || valid < 0.5) ? 0.0
        : totalDist + sqrt((bx2-prevData.x)*(bx2-prevData.x) + (by2-prevData.y)*(by2-prevData.y));
    fragColor = vec4(bx2, by2, newDist, 1.0);
}`}static nonlinearAccumulate(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform sampler2D u_distanceTexture;
uniform bool u_reset;
...
void main() {
    ...
    float newDist = (u_reset || valid < 0.5) ? 0.0
        : totalDist + sqrt((bx2-prevData.x)*(bx2-prevData.x) + (by2-prevData.y)*(by2-prevData.y));
    fragColor = vec4(bx2, by2, newDist, 1.0);
}`}static rigidDivergenceStep(){return`${v}
uniform sampler2D u_stateTexture;
...
}`}static elasticDivergenceStep(){return`${v}
...
void main() {
    ...
    outPertB = newPB;
    divergenceData = vec4(iter, hasDiv, 0.0, 1.0);
}`}static nonlinearDivergenceStep(){return`${v}
uniform sampler2D u_baseTextureA;
uniform sampler2D u_baseTextureB;
uniform sampler2D u_pertTextureA;
uniform sampler2D u_pertTextureB;
...
void main() {
    ...
    outPertB = newPB;
    divergenceData = vec4(iter, hasDiv, 0.0, 1.0);
}`}static rigidDivDistanceInit(){return`${v}
uniform vec4 u_c00;
uniform vec4 u_c10;
uniform vec4 u_c01;
uniform vec4 u_c11;
...
}`}static elasticDivDistanceInit(){return`${v}
uniform vec4 u_cA00;
uniform vec4 u_cA10;
uniform vec4 u_cA01;
uniform vec4 u_cA11;
...
void main() {
    ...
    divergenceData = vec4(bx2, by2, 0.0, 0.0);
}`}static nonlinearDivDistanceInit(){return this.elasticDivDistanceInit()}static rigidDivDistanceStep(){return`${v}
uniform sampler2D u_stateTexture;
uniform sampler2D u_perturbedTexture;
uniform sampler2D u_divergenceTexture;
uniform float u_dt;
...
}`}static elasticDivDistanceStep(){return`${v}
uniform sampler2D u_baseTextureA;
uniform sampler2D u_baseTextureB;
uniform sampler2D u_pertTextureA;
uniform sampler2D u_pertTextureB;
...
void main() {
    ...
    divergenceData = vec4(bx2, by2, totalDist, hasDiv);
}`}static nonlinearDivDistanceStep(){return`${v}
uniform sampler2D u_baseTextureA;
uniform sampler2D u_baseTextureB;
uniform sampler2D u_pertTextureA;
uniform sampler2D u_pertTextureB;
...
}`}static previewElasticPhysicsLoop(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform float u_dt;
uniform int u_steps;
...
void main() {
    ...
    outStateA = sa;
    outStateB = sb;
}`}static previewNonlinearPhysicsLoop(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform float u_dt;
uniform int u_steps;
...
void main() {
    ...
    outStateA = sa;
    outStateB = sb;
}`}static previewRigidInit(){return`${v}
...
}`}static previewElasticInit(){return`${v}
...
}`}static previewNonlinearInit(){return this.previewElasticInit()}static previewRigidDivCheck(){return`${v}
...
}`}static previewElasticDivCheck(){return`${v}
...
void main() {
    ...
    fragColor = vec4(diverged, countAtDiv, 0.0, 0.0);
}`}static previewNonlinearDivCheck(){return this.previewElasticDivCheck()}static previewRigidTrailAppend(){return`${v}
uniform sampler2D u_baseState;
in vec2 v_uv;
out vec4 fragColor;
...
}`}static previewElasticTrailAppend(){return`${v}
uniform sampler2D u_baseA;
uniform sampler2D u_baseB;
in vec2 v_uv;
out vec4 fragColor;
...
void main() {
    ...
    fragColor = vec4(x2, y2, 0.0, 1.0);
}`}static previewNonlinearTrailAppend(){return`${v}
uniform sampler2D u_baseA;
uniform sampler2D u_baseB;
in vec2 v_uv;
out vec4 fragColor;
...
`}static previewRigidRender(){return`${v}
uniform sampler2D u_baseState;
uniform sampler2D u_pertState;
uniform sampler2D u_meta;
...
}`}static previewElasticRender(){return`${v}
uniform sampler2D u_baseA;
uniform sampler2D u_baseB;
uniform sampler2D u_pertA;
uniform sampler2D u_pertB;
...
}`}static batchRigidInit(){return`${v}
...
void main() {
    ...
    fragColor = state;
}`}static batchElasticInit(){return`${v}
...
void main() {
    ...
    outA = a;
    outB = b;
}`}static batchNonlinearInit(){return this.batchElasticInit()}static batchRigidPhysics(){return`${v}
uniform sampler2D u_stateTexture;
uniform float u_dt;
uniform int u_steps;
uniform float u_m1;
...
void main() {
    ...
    fragColor = vec4(theta1, omega1, theta2, omega2);
}`}static batchElasticPhysics(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform float u_dt;
uniform int u_steps;
...
void main() {
    ...
    outStateA = sa;
    outStateB = sb;
}`}static batchNonlinearPhysics(){return`${v}
uniform sampler2D u_stateTextureA;
uniform sampler2D u_stateTextureB;
uniform float u_dt;
uniform int u_steps;
